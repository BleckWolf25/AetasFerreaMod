...
public class FallDamageEventHandler {
    ...
    public static void onChickenFall(LivingFallEvent event) {
        ...
        // Execute damage directly since Chicken vanilla logic explicitly rejects fall damage calls
        if (damage > 0) {
            chicken.hurt(Objects.requireNonNull(chicken.damageSources().fall()), (float) damage);
        }
